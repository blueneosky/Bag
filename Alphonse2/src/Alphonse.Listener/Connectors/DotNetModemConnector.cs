...
public class DotNetModemConnector : ModemConnector
{
    private static DateTimeOffset lastClosedTime = DateTimeOffset.MinValue;

    private readonly AsyncAccessBox<ModemContext> _context = new(new());

    public DotNetModemConnector(
        ILogger<DotNetModemConnector> logger,
    ...
    public override Task OpenAsync() => this._context.UseAsync(async context =>
    {
        if (context.Connector is not null)
            throw new InvalidOperationException("Already opened");

        this.Logger.LogInformation("Opening the modem on '{ModemPort}' ...", this.ModemPort);
        var connector = new SerialPort(this.ModemPort, 9600)
        {
            NewLine = "\r\n",
        ...
        // required some time after a close (SerialPort)
        var closeOpenDelay = DateTimeOffset.Now - DotNetModemConnector.lastClosedTime;
        if (closeOpenDelay < TimeSpan.FromMilliseconds(500))
            await Task.Delay(TimeSpan.FromMilliseconds(500) - closeOpenDelay);
    ...
    private async Task<TaskCompletionSource?> SetupListenAsync(IModemDataDispatcher listener, ModemContext context, CancellationToken token)
    {
        ...
        {
            if (!(sender is SerialPort connector) || connector != context.Connector)
                return;
        ...
        void OnErrorReceived(object sender, SerialErrorReceivedEventArgs e)
        {
            ...
            {
                // create a contexte
                throw new InvalidDataException($"[{nameof(SerialPort)}] Error received: '{e.EventType.ToString()}'");
            }
            catch (Exception ex)
            {
    ...
    public override void Close()
    {
        ...
        DotNetModemConnector.lastClosedTime = DateTimeOffset.Now;
    }

    private record ModemContext
    {
        public SerialPort? Connector { get; set; }
