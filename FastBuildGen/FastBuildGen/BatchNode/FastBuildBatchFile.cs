...
namespace FastBuildGen.BatchNode
{
    ...
    internal class FastBuildBatchFile : BatchFileBase
    {
        ...
        private IEnumerable<IEnumerable<FBTarget>> AllFBTargetss
        {
            get
            {
        ...
        private FBTarget GetParamDescription(string keyWord)
        {
            FBTarget result = AllFBTargetss
                .SelectMany(e => e)
        ...
        public Dictionary<string, LiteralBatch> LiteralFBTargetByKeyWords
        {
            get
            {
                if (_literalParamDescriptionByKeyWordsCache == null)
                {
                    _literalParamDescriptionByKeyWordsCache = AllFBTargetss
                        .SelectMany(e => e)
                        .ToDictionary(
                            pd => pd.Keyword
        ...
        public IEnumerable<string> HelpLines
        {
            get
            {
                ...
                {
                    IEnumerable<FBTarget> baseParamDescriptions = BaseParamDescriptions.ToArray();
                    ...
                    _helpLinesCache = baseParamDescriptions.Select(pd => TranslateFBTargetToHelpLine(pd, padingBaseParamDescriptions));
                    if (heoParamDescriptions.Any())
                    {
                        _helpLinesCache = _helpLinesCache
                            ...
                            .Concat(heoParamDescriptions.Select(pd => TranslateFBTargetToHelpLine(pd, padingHeoParamDescriptions)));
                    }
        ...
        private string TranslateFBTargetToHelpLine(FBTarget fbTarget, int pading)
        {
            return String.Format("{0} : {1}", fbTarget.SwitchKeyword.PadRight(pading, ' '), fbTarget.HelpText);
        }
        ...
        private IEnumerable<BatchStatementNodeBase> ParametersAnalyseInitialisationCmds
        {
            ...
            {
                bool isFirst = true;
                foreach (var paramDescriptions in AllFBTargetss)
                {
                    ...
                    {
                        LiteralBatch literal = LiteralFBTargetByKeyWords[paramDescription.Keyword];
        ...
        private BlocMacroFileNode ParametersAnalyseParsingCmds
        {
            get
            {
                ...
                bool isFirst = true;
                foreach (var fbTargets in this.AllFBTargetss)
                {
                    if (fbTargets.Any())
                    {
                        if (isFirst)
                        ...
                        foreach (var fbTarget in fbTargets)
                        {
                            bool isInnerFirst = true;
                            FBMacroSolutionTarget fbMacroSolutionTarget = fbTarget as FBMacroSolutionTarget;
                            if (fbMacroSolutionTarget == null)
                            {
                                blocMacro.Add(GetParametersAnalyseParsingCmd(fbTarget.SwitchKeyword, fbTarget, leftIfTest));
                            }
                            else
                            {
                                if (isInnerFirst)
                                ...
                                string switchKeyword = fbMacroSolutionTarget.SwitchKeyword;
                                IEnumerable<FBTarget> fbSolutionTargets = fbMacroSolutionTarget.SolutionTargetIds
                                    .Join(this.SolutionTargets, id => id, st => st.Id, (id, st) => st);
                                blocMacro.AddRange(fbSolutionTargets.Select(pd => GetParametersAnalyseParsingCmd(switchKeyword, pd, leftIfTest)));
                            }
                        }
                    }
                }
        ...
        private BatchFileNodeBase GetParametersAnalyseParsingCmd(string switchKeyword, FBTarget fbTarget, BatchExpressionBase leftIfTest)
        {
            ...
            LiteralBatch literal = LiteralFBTargetByKeyWords[fbTarget.Keyword];
        ...
        public BlocMacroFileNode ParametersAnalyseUsageAndVer
        {
            get
            {
                ...
                LiteralBooleanExpression literalValueHelp = LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchHelp].LiteralBoolean;
                LiteralBooleanExpression literalValueVer = LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchVer].LiteralBoolean;
        ...
        public BlocMacroFileNode FastBuildCore
        {
            get
            {
                ...
                blocMacro.Add(new IfCmd(new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchWait]), new PauseCmd()));
        ...
        public BlocMacroFileNode FastBuildCoreMSBuildEnvInitConst
        {
            get
            {
                ...
                foreach (FBSolutionTarget fbSolutionTarget in SolutionTargets)
                {
                    blocMacro.Add(new SetExpressionCmd(LiteralModuleMSBuildTargetByKeyWords[fbSolutionTarget.Keyword], new ValueExpression(fbSolutionTarget.MSBuildTarget)));
                }
        ...
        public BlocMacroFileNode FastBuildCoreMSBuildEnvInitMSBuildConfig
        {
            get
            {
                ...
                blocMacro.Add(new IfCmd(
                    new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchDsac])
                    , new SetExpressionCmd(LiteralMSBuildConfiguration, new ValueExpression(_fbModel.ValueMSBuildConfigurationDsac()))
                    ));
                blocMacro.Add(new IfCmd(
                    new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchRelease])
...
#warning TODO ALPHA BETA point - retirer le cas win32
                    ...
                    .Select(pdm => new Tuple<BooleanExpressionBase, BatchExpressionBase>(
                        LiteralFBTargetByKeyWords[pdm.Keyword].LiteralBoolean
...
#warning TODO ALPHA BETA point - retirer le cas win32
        ...
        private MSBuildCliMacro GetFastBuildCoreMSBuildEnvInitMSBuildConfigMSBuildCliMacro(LiteralBatch literalMSBuildCli, BatchExpressionBase platform, IEnumerable<Tuple<BooleanExpressionBase, BatchExpressionBase>> targets)
        {
            MSBuildCliMacro result = new MSBuildCliMacro(
                ...
                , TrueValueExpression.ValueExpression
                , LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchPara].LiteralBoolean
                , LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchQuiet].LiteralBoolean
                , new QuotedValueExpression(LiteralMSBuildConfiguration.LiteralValue)
                , LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchFxcop].LiteralBoolean
                , LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchNowarn].LiteralBoolean
                , LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchRebuild].LiteralBoolean
                , platform
                ...
                , new Tuple<BooleanExpressionBase, BatchExpressionBase>(
                    LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchForceOutputDirPath].LiteralBoolean
        ...
        public BlocMacroFileNode FastBuildCoreSGenPlus
        {
            get
            {
                ...
                blocMacro.Add(new IfCmd(
                    new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchNosgp])
                    , new SetFalseCmd(LiteralSGenPlusNeedRun)
                    ));
        ...
        public BlocMacroFileNode SubFunctionsKillHeoVsHost
        {
            get
            {
                ...
                IfMacro ifMacro = new IfMacro(
                    new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchVshost])
                    , nodeThen
                    , null
                );
        ...
        public BlocMacroFileNode SubFunctionsKillHeo
        {
            get
            {
                ...
                IfMacro ifMacro = new IfMacro(
                    new IsTrueConditional(LiteralFBTargetByKeyWords[FastBuildParamModel.ConstKeywordParamSwitchKillheo])
                    , nodeThen
                    , null
                );
